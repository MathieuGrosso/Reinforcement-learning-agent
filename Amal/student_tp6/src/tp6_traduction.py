...
class train_global(nn.Module):
        ...
        for idx,batch in enumerate(traindata):
            ...
            if batch_size==BATCH_SIZE:
                ...
                if use_teacher_forcing:
                    ...
                    ##mot suivant: 
                    #word_embedded = self.embedder(word_next)
    ...
    def fit(self,traindata,testdata,encode_params,decode_params,batch_size,hidden_size,nb_classes,nb_oov,test_data=None,validation_data=None, start_epoch=0, n_epochs=50, lr=0.001, verbose=10,ckpt=None):
        ...
        if self.opt=="SGD":
            encoder_optimizer = optim.SGD(encode_params, lr=lr) # try with adam after
        ...
        with torch.no_grad():
            ...
            for step in range(length-1):
                ...
                for i in range(k):
                    ...
                    # for idx,j in enumerate(current_prob): 
                    #     k_probas[i][idx]+=1
                    # for idx,j in enumerate(current_indx):
                    #     k_strings[i][idx].append(j) # liste avec 9 phrases, on n eva en garder que 3. 
                ...
                probas,indices = torch.topk(new_prob.flatten(),k=3,dim=0) #on obtient l'index des 3 meilleurs probas. 
                three_best_indx = [i.item() for i in indices]
                ic(three_best_indx)
                indx=[new_indx[i] for i in three_best_indx]


                # for i in three_best_indx: 
                #     if i<3:
    ...
    def generate_last_beam_2(self,testdata,k=3):
        str = vocFra.SOS  #initialisation de la str avec le caractère start
        k_strings = [[] for i in range(k)]  #initialise les k string à la valeur donnée en entrée
        k_probas = [0] * k #initialise les k logprobas à 0
        k_output=[]
...
if NoSegmentation : 
    ...
    generation_beam = training.generate_last_beam_2(testdata=datatest,k=3)
...
##instanciate data: 
    ...
    generation_beam = training.generate_last_beam_2(testdata=datatest_ngram,k=3)
